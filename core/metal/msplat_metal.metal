...
kernel void rasterize_backward_kernel(
    ...
    for (int b = first_batch; b < num_batches; ++b) {
        ...
        for (int t = max(0,batch_end - warp_bin_final); t < batch_size; ++t) {
            ...
            if(valid){
                // Walk the transmittance back to its pre-gaussian value. The forward
                // multiplied T by (1 - alpha) using the same clamped alpha, so this
                // must run for every contributing gaussian — skipping it leaves T too
                // small for every nearer gaussian on this pixel.
                ...
                const float v_alpha = dot(fma(rgb, T, fma(-buffer, ra, -ra * T_final_bg)), v_out);
                // update the running sum
                buffer = fma(rgb, fac, buffer);

                // alpha = min(0.999, opac * exp(-sigma)). Where the cap binds, alpha is
                // constant in both sigma and opac, so every gradient routed through
                // sigma is exactly zero. The color gradient above is not.
                if (alpha < 0.999f) {
...
kernel void rasterize_backward_chunked_kernel(
    ...
    for (int b = first_batch; b < num_batches; ++b) {
        ...
        for (int t = max(0, batch_end - warp_bin_final); t < batch_size; ++t) {
            ...
            if (valid) {
                // See nd_rasterize_backward path: T must be walked back for every
                // contributing gaussian, including one whose alpha hit the 0.999 cap.
                float ra = 1.f / (1.f - alpha);
                T *= ra;
                const float fac = alpha * T;
                v_rgb_local = fac * v_out;
                ...
                // Where the cap binds, alpha is constant in sigma and opac, so every
                // sigma-routed gradient is exactly zero.
                if (alpha < 0.999f) {
                    const float v_sigma = -alpha * v_alpha;
                    v_conic_local = (0.5f * v_sigma) * float3(delta.x * delta.x,
                                                               delta.x * delta.y,
