...
// p_view: pre-computed view-space position (from clip_near_plane), avoids redundant transform
// Exploits J sparsity (5/9 nonzero) and only computes upper-left 2x2 of cov2d
float3 project_cov3d_ewa(
    ...
    // clip so that the covariance doesn't explode at edges
...
inline bool compute_cov2d_bounds(
    ...
    // max eigenvalue is always b + disc (disc >= 0)
